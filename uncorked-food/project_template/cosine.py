...
inverted_index_wine, docid_to_wine_title = build_inverted_index_wine(wine_data)
...
def search(query, searchType):
  ...
  if searchType == SearchType.WINE:
    output = index_search_cosine_sim(query, inverted_index_wine, doc_norms_wine, idf_dict_wine, docid_to_wine_title)

  elif searchType == SearchType.FOOD:
    output = index_search_cosine_sim(query, inverted_index_food, doc_norms_food, idf_dict_food, recipe_id_to_title)
